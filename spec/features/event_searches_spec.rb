...
RSpec.feature "EventSearches", type: :feature do
  ...
  context 'Searching for events' do
    Steps 'To search for events' do
      ...
      And 'I can see only events that are filtered after a certain date ' do
        expect(page).to have_content('Canned Food Event')
        expect(page).to have_content('Food Donation')
        expect(page).not_to have_content('Blood Drive')
      end
      Then 'I can go back to the events index and filter events in a date range' do
        visit '/events'
        page.select('2017', from: 'end_time_year')
        page.select('January', from: 'end_time_month')
        page.select('1', from: 'end_time_day')
        page.select('2017', from: 'end_time_year')
        page.select('February', from: 'end_time_month')
        page.select('1', from: 'end_time_day')
        click_button 'Search All'
      end
      And 'I can see only events that are filtered after a certain date ' do
